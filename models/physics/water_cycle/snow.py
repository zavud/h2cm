...
def compute_snow_acc(prec_t: torch.Tensor, beta_snow: torch.Tensor, tair_t_celsius: torch.Tensor):
    ...
    Returns: snow_acc_t containing snow accumulations (mm/day) at the current time step. Torch tensor of shape (batch_size, 1)
    """
...
def compute_snow_melt(swe_t_prev: torch.Tensor, tair_t_celsius: torch.Tensor, alpha_snow_melt_t: torch.Tensor):
    ...
    Returns: snow_melt_t containing the amount of snow (mm/day) melted at the current time step. Torch tensor of shape (batch_size, 1)
    """
...
def update_swe(swe_t_prev: torch.Tensor, snow_acc_t: torch.Tensor, snow_melt_t: torch.Tensor):
    ...
    Arguments:
    swe_t_prev: Snow water equivalent (mm) at the previous time step t-1. Torch tensor of shape (batch_size, 1).
    snow_acc_t: Snow accumulation (mm/day) at the current time step t. Torch tensor of shape (batch_size, 1)
    snow_melt_t: Snow melt (mm/day) at the current time step t. Torch tensor of shape (batch_size, 1)
